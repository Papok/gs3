...
class active_html_base {
  constructor(
    ...
    this.attributes = new Map([["id", this.id]]);
  ...
  link_handlers() {
    let namespace = ".hgs3";
    $(this.selector).off(namespace);
    for (let [event, f] of this.events) {
      let event_name = `${event}${namespace}`;
      $(this.selector).on(event_name, f);
    }
  ...
  add_attr(attribute_name, value) {
    let extra_attribute = [[attribute_name, value]];
    concat_attributes(this.attributes, extra_attribute);
    $(this.selector).attr(attribute_name, value);
  }
  ...
  toggle() {
    if (this.attributes.get("hidden") === true) {
      this.unhide();
    } else {
      this.hide();
    }
...
class option extends active_html_base {
  constructor(
    ...
    let tag = "option";
    let inner = label;
    let attributes = new Map([["value", value]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, tag, inner, attributes);
    this._label = label;
    this._value = value;
...
class rich_select extends select {
  constructor(
    parent = "autoparent",
    options = [new logic.rich_option_item()],
    value = 0,
    extra_attributes = []
  ) {
    let attributes = new Map();
  ...
  _add_option(option_) {
    ...
    let background_color_set_string =
...
class label extends active_html_base {
  constructor(
    ...
    let tag = "label";
    let inner = text;
    let attributes = new Map([["for", dest]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, tag, inner, attributes);
    this.text = text;
  }
  ...
  draw() {
    if (this.attributes.get("for") === undefined) {
      console.error(
        "<label> element " + this.id + " has not defined 'for' attrubute"
      );
    }
    super.draw();
  }
...
class text_input extends input {
  constructor(parent, extra_attributes = []) {
    let attributes = new Map([["type", "text"]]);
    concat_attributes(attributes, extra_attributes);
...
class password_input extends input {
  constructor(parent, extra_attributes = []) {
    let attributes = new Map([["type", "password"]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, attributes);
  }
}
...
class number_input extends input {
  constructor(parent, extra_attributes = []) {
    let attributes = new Map([["type", "number"]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, attributes);
  }
}
...
class date_input extends input {
  constructor(parent, date, extra_attributes = []) {
    ...
    let attributes = new Map([["type", "date"], ["value", date_string]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, attributes);
  }
...
class submit_input extends active_html_base {
  constructor(parent, text = "Submit", extra_attributes = []) {
    let tag = "input";
    let inner = "";
    let attributes = new Map([["type", "submit"], ["value", text]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, tag, inner, attributes);
    if (parent.tag !== "form" && parent.tag !== undefined) {
      console.warn(
        "submit_input elements should be inside form elements. (" +
          this.id +
          ")"
      );
    }
...
class form extends active_html_base {
  ...
  set_values(data) {
    for (let [field, control] of Object.entries(this.fields.reference)) {
...
class bs_form_group extends active_html_base {
  constructor(
    parent = "autoparent",
    label_text = "void",
    control = new active_html_base(),
    extra_attributes = []
  ) {
    let tag = "div";
    let form_label =
      typeof label_text == "string"
        ? new label(undefined, label_text, control.get_id())
        : label_text;
    let inner = [form_label, control];
    let attributes = new Map([["class", "form-group"]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, tag, inner, attributes);
...
class bs_row extends div {
  constructor(
    ...
    let attributes = new Map([["class", "row"]]);
    concat_attributes(attributes, extra_attributes);
    ...
    for (let element_idx = 0; element_idx < elements.length; ++element_idx) {
      ...
      this.inner.push(
        new div(this, elements[element_idx], [
...
class bs_dd_button_item extends button {
  constructor(parent = "autoparent", text = "void", extra_attributes = []) {
    let attributes = new Map([["class", "dropdown-item"], ["type", "button"]]);
    concat_attributes(attributes, extra_attributes);
...
class bs_dd_menu extends active_html_base {
  constructor(
    parent = "autoparent",
    options = [new bs_dd_button_item()],
    extra_attributes = []
  ) {
    let tag = "div";
    let inner = options;
    let attributes = new Map([["class", "dropdown-menu"]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, tag, inner, attributes);
...
class bs_dd extends active_html_base {
  constructor(
    parent = "autoparent",
    head = new bs_dd_button(),
    options_list = new bs_dd_menu(),
    extra_attributes = []
  ) {
    let tag = "div";
    let inner = [head, options_list];
    let attributes = new Map([["class", "dropdown"]]);
    concat_attributes(attributes, extra_attributes);
    super(parent, tag, inner, attributes);
    this.head = head;
  }
...
class bs_dd_value_on_button extends bs_dd {
  constructor(parent, menu, selected_option = 0, extra_attributes = []) {
    super(
      ...
      new bs_dd_button(
        "autoparent",
        menu.get_option(selected_option).get_value(),
        new Map([["class", "btn-block"]])
      ),
      menu
    );
    for (let option of menu.get_options()) {
      option.set_handler("click", () => {
        this.set_head_text(option.get_text());
      });
    }
  }
}
...
class bs_dd_text_input extends text_input {
  ...
  link() {
    ...
    on_keypress.call(this, "13", function() {
      ...
      if (
        self.parent.list.items.indexOf(self.selected_item) < 0 &&
        self.selected_item != ""
      ) {
        /*self.*/
...
class bs_items_list extends active_html_base {
  constructor(parent, items, extra_attributes = []) {
    ...
    items.forEach(function(item) {
      var item_object =
        typeof item == "string" ? new list_item(self, item) : item; // atención! si paso un objeto item_list, este va a quedar con su propio antecesor.
...
class bs_dd_items_list extends bs_items_list {
  constructor(parent, items, extra_attributes = []) {
    super(
      parent,
      items,
      extra_attributes.concat([new attribute("class", "dropdown-menu")])
    );
    ...
    this.items.forEach(function(item) {
      ...
      $(item.selector).click(function(event) {
        ...
        if (event.metaKey || event.ctrlKey) {
          ...
          $(self.parent.input_box.selector).attr(
            "placeholder",
            self.parent.input_box.placeholder
          );
        }
      });
    });
...
class bs_dd2 extends active_html_base {
  constructor(parent, items, selected_item = items[0], extra_attributes = []) {
    super(parent);
    this.extra_attributes = extra_attributes;
    this.input_box = new bs_dd_text_input(this, selected_item, [
  ...
  update_list(items) {
    ...
    this.list = new bs_dd_items_list(
      this,
      items /*, [new attribute("class", "form-control")]*/
    );
...
class bs_column extends active_html_base {
  constructor(parent, size = "", inner = "", extra_attributes = []) {
    ...
    let attributes = [new attribute("class", this.size)];
    this.full_attributes = attributes.concat(this.extra_attributes);
  }
}
...
class colorpicker extends div {
  constructor(parent) {
    let e_1_1 = new active_html_base(
      "autoparent",
      "div",
      "",
      new Map([["class", "colorPicker_colorInner"]])
    );
    let e_1 = new active_html_base(
      "autoparent",
      "a",
      e_1_1,
      new Map([["class", "colorPicker_color"]])
    );
    let e_2 = new active_html_base(
      "autoparent",
      "div",
      "",
      new Map([["class", "colorPicker_track"]])
    );
    let e_3_1 = new active_html_base("autoparent", "li", "");
    let e_3 = new active_html_base(
      "autoparent",
      "ul",
      e_3_1,
      new Map([["class", "colorPicker_dropdown"]])
    );
    let e_4 = new active_html_base(
      "autoparent",
      "input",
      "",
      new Map([["type", "hidden"], ["class", "colorPicker_colorInput"]])
    );
    ...
    super(parent, inner, new Map([["class", "colorPicker_main"]]));
  ...
  link_handlers() {
    ...
    if (this.color == undefined) {
      this.color = hsl2rgb("hsl(0, 100%, 75%)");
    }
    this.picker.setColor(this.color);

    //this.picker el "this.picker" no parece ser parte del objeto sino de esta funcion. Lo mismo para "set value↓↓↓"
  }
  ...
  set value(color_string) {
    if (color_string.startsWith("hsl")) {
      this.color = hsl2rgb(color_string);
    } else if (color_string.startsWith("rgb")) {
      this.color = color_string;
    } else {
      console.error("bad color format → " + color_string);
    }
  }
}
